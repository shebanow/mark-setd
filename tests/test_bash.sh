...
test_cd_number() {
    ...
    # Get expected directory from history
    local output=$(cd -l 2>&1 | head -n 2 | tail -n 1)
...
# Test 9: Navigate to directory with special characters (if supported)
if [ -d "$TEST_ROOT/dir#with#hash" ]; then
    test_cd "$TEST_ROOT/dir#with#hash" "$TEST_ROOT/dir#with#hash" "cd to directory with hash"
fi
